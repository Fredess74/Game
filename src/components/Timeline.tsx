    ...
    <div className="flex flex-col h-full bg-[#1e293b] text-gray-200 select-none border-t border-gray-700">
       ...
       <div className="h-10 border-b border-gray-700 flex items-center justify-between px-4 bg-[#0f172a]">
          <div className="flex items-center gap-2">
             <button onClick={() => setTime(0)} aria-label="Skip to start" title="Skip to start" className="p-1 hover:text-white text-gray-400"><SkipBack size={16} /></button>
             <button onClick={handlePlayToggle} aria-label={isPlaying ? 'Pause' : 'Play'} title={isPlaying ? 'Pause' : 'Play'} className="p-1 hover:text-white text-[#4ade80]">
                {isPlaying ? <Pause size={16} fill="currentColor" /> : <Play size={16} fill="currentColor" />}
             </button>
             <button onClick={() => setTime(duration)} aria-label="Skip to end" title="Skip to end" className="p-1 hover:text-white text-gray-400"><SkipForward size={16} /></button>
