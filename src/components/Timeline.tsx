    ...
    <div className="flex flex-col h-full bg-[#1e293b] text-gray-200 select-none border-t border-gray-700">
       ...
       <div className="h-10 border-b border-gray-700 flex items-center justify-between px-4 bg-[#0f172a]">
          <div className="flex items-center gap-2">
             <button onClick={() => setTime(0)} className="p-1 hover:text-white text-gray-400 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#4ade80] rounded" aria-label="Skip to beginning" title="Skip to beginning"><SkipBack size={16} /></button>
             <button onClick={handlePlayToggle} className="p-1 hover:text-white text-[#4ade80] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#4ade80] rounded" aria-label={isPlaying ? "Pause" : "Play"} title={isPlaying ? "Pause" : "Play"}>
                {isPlaying ? <Pause size={16} fill="currentColor" /> : <Play size={16} fill="currentColor" />}
             </button>
             <button onClick={() => setTime(duration)} className="p-1 hover:text-white text-gray-400 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#4ade80] rounded" aria-label="Skip to end" title="Skip to end"><SkipForward size={16} /></button>
          ...
          <div className="flex items-center gap-2">
             <button
                ...
                className="flex items-center gap-1 text-xs px-2 py-1 rounded bg-gray-700 hover:bg-[#4ade80] hover:text-black disabled:opacity-50 disabled:cursor-not-allowed transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#4ade80]"
             >
