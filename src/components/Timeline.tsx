    ...
    <div className="flex flex-col h-full bg-[#1e293b] text-gray-200 select-none border-t border-gray-700">
       ...
       <div className="h-10 border-b border-gray-700 flex items-center justify-between px-4 bg-[#0f172a]">
          <div className="flex items-center gap-2">
             <button aria-label="Skip to Start" title="Skip to Start" onClick={() => setTime(0)} className="p-1 hover:text-white text-gray-400 focus-visible:ring-2 focus-visible:ring-editor-accent focus-visible:outline-none rounded"><SkipBack size={16} /></button>
             <button aria-label={isPlaying ? "Pause" : "Play"} title={isPlaying ? "Pause" : "Play"} onClick={handlePlayToggle} className="p-1 hover:text-white text-[#4ade80] focus-visible:ring-2 focus-visible:ring-editor-accent focus-visible:outline-none rounded">
                {isPlaying ? <Pause size={16} fill="currentColor" /> : <Play size={16} fill="currentColor" />}
             </button>
             <button aria-label="Skip to End" title="Skip to End" onClick={() => setTime(duration)} className="p-1 hover:text-white text-gray-400 focus-visible:ring-2 focus-visible:ring-editor-accent focus-visible:outline-none rounded"><SkipForward size={16} /></button>
             <div className="w-px h-4 bg-gray-700 mx-2" />
             <div className="font-mono text-xs text-[#4ade80]" aria-live="polite">
          ...
          <div className="flex items-center gap-2">
             <button
                ...
                disabled={!selectedId}
                title={selectedId ? "Add Keyframe" : "Select an actor to add keyframes"}
                className="flex items-center gap-1 text-xs px-2 py-1 rounded bg-gray-700 hover:bg-[#4ade80] hover:text-black disabled:opacity-50 disabled:cursor-not-allowed focus-visible:ring-2 focus-visible:ring-editor-accent focus-visible:outline-none transition-colors"
             >
