    ...
    <div className="flex flex-col h-full bg-[#1e293b] text-gray-200 select-none border-t border-gray-700">
       ...
       <div className="h-10 border-b border-gray-700 flex items-center justify-between px-4 bg-[#0f172a]">
          <div className="flex items-center gap-2">
             <button onClick={() => setTime(0)} title="Skip to Start" aria-label="Skip to Start" className="p-1 hover:text-white text-gray-400 rounded focus-visible:ring-2 focus-visible:ring-editor-accent focus-visible:outline-none"><SkipBack size={16} /></button>
             <button onClick={handlePlayToggle} title={isPlaying ? "Pause" : "Play"} aria-label={isPlaying ? "Pause" : "Play"} className="p-1 hover:text-white text-[#4ade80] rounded focus-visible:ring-2 focus-visible:ring-editor-accent focus-visible:outline-none">
                {isPlaying ? <Pause size={16} fill="currentColor" /> : <Play size={16} fill="currentColor" />}
             </button>
             <button onClick={() => setTime(duration)} title="Skip to End" aria-label="Skip to End" className="p-1 hover:text-white text-gray-400 rounded focus-visible:ring-2 focus-visible:ring-editor-accent focus-visible:outline-none"><SkipForward size={16} /></button>
